...
void ATestCharacter::TestLineTrace()
{
	...
	QueryParams.AddIgnoredActor(this);	// 자기 자신은 체크 안 함
	...
	if (bHit)
	{
		...
		{
			FGameplayEffectContextHandle Context = TargetASC->MakeEffectContext();	// 컨텍스트 만들기
			Context.AddHitResult(HitResult);				// 라인트레이스의 히트 결과 전달
			Context.AddInstigator(GetInstigator(), this);	// 인스티게이터 지정

			FGameplayEffectSpecHandle Spec = TargetASC->MakeOutgoingSpec(	// 컨텍스트 기반으로 스펙만들기
				TestHitEffectClass,
				1,
				Context
			);
			if (Spec.IsValid())
			{
				...
				TargetASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());	// 이펙트 적용
			}
		}
	}
	else
